...
const OrderList = () => {
  ...
  if (loading) {
    return (
      <main className="flex-1 overflow-hidden w-full max-w-[950px] mx-auto px-2">
  ...
  return (
    <main className="flex-1 overflow-hidden w-full max-w-[950px] mx-auto px-2">
      ...
      <div className="cursor-pointer bg-white w-full sticky top-0 z-10 border-b">
        <div className="flex flex-row overflow-x-auto whitespace-nowrap">
            {tabs.map((tab) => (
            <button
                key={tab}
                className={`flex-shrink-0 w-auto min-w-max sm:w-1/6 px-4 py-3 text-center text-sm sm:text-base hover:border-b-2 hover:border-[#0D5CB6] ${
                activeTab === tab
      ...
      <div className="w-full relative my-3">
        <input
          ...
          className="transition-[border-color,box-shadow] duration-150 ease-in-out h-9 w-full rounded-md px-3 py-2.5 leading-5 outline-none flex-1 border border-[#C4C4CF] pr-28"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
        />
        <button className="absolute right-2 top-1/2 -translate-y-1/2 text-[#0B74E5] py-0 px-2 sm:px-[6px] pl-2 sm:pl-4 border-l-2 border-[#DDDDE3] cursor-pointer text-sm">
          Tìm đơn hàng
        </button>
      </div>
      ...
      {filteredOrders.length === 0 ? (
        <div className="text-center text-gray-500 mt-10 p-4 bg-white rounded-md">
          Chưa có đơn hàng
        </div>
      ) : (
        <div className="bg-white rounded-md text-[13px] mb-5 p-2 sm:p-4">
            ...
            >
              ...
              <div className="space-y-3 mb-4">
                {order.orderItems.map((item, i) => (
                  <div key={i} className="flex flex-row py-4 border-b border-[#EBEBF0] last:border-b-0 justify-between items-start gap-3">
              ...
              <div className="flex flex-col items-end mt-3 w-full">
                <div className="text-base sm:text-[17px] flex mb-3">
                ...
                <div className="flex w-full sm:w-auto">
                  <button onClick={() => handleViewDetail(order.orderId)} className="py-2 px-4 h-9 rounded-md border border-[#0B74E5] text-sm font-normal leading-normal text-[#0B74E5] flex justify-center items-center cursor-pointer w-full sm:w-auto ml-0 sm:ml-2">
                    Xem chi tiết
                  </button>
                </div>
              </div>
