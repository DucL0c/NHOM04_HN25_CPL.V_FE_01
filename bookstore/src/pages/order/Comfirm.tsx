...
export default function Confirm() {
  ...
  // Tính tiền (nếu total không truyền từ checkout thì tự tính)
  const subtotal = items.reduce(
    (acc, it) => acc + toNumberVND(it.item.price) * it.quantity,
    0
  );
        ...
        <aside className="lg:col-span-4">
          <div className="bg-white rounded-lg shadow-sm p-4">
            ...
            <div className="mt-3 flex gap-3 items-start">
              <img
                src={first.item.image || "/placeholder.svg?height=56&width=56"}
                className="w-12 h-12 rounded object-cover"
              />
              <div className="flex-1">
                <div className="line-clamp-2 text-sm">{first.item.name}</div>
                <div className="text-xs text-gray-500">
                  SL: x{first.quantity}
                  {more > 0 ? ` • +${more} sản phẩm khác` : ""}
                </div>
              </div>
            </div>
          </div>
